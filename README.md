...
**Продакшен:** [https://ii-doktor.ru/](https://ii-doktor.ru/)
...
| **Анализы** | Ввод лабораторных показателей, тренды относительно предыдущих значений; загрузка файлов с превью и поворотом JPG/PNG/WEBP перед OCR |
...
| `DELETE` | `/api/documents/{id}` | JWT | Удаление своего документа |
...
| `FRONTEND_URL` | `https://ii-doktor.ru` | Базовый URL для ссылок сброса пароля |
...
> Подробная пошаговая инструкция переноса приложения с локальной машины на сервер —
...
curl -sk https://ii-doktor.ru/health
```
